...
// Formats content with a bold "Qodo Highlight" title
// Returns HTML string with the title and content
function formatContentWithTitle(content) {
  return `
    <div style="text-align: left;">
      <div style="font-weight: 900; font-size: 20px; margin-bottom: 8px;">Qodo Highlight</div>
      <div>${content}</div>
    </div>
  `;
}
...
function createTippyInstance(targetElement, content) {
  return window.tippy(targetElement, {
    ...TIPPY_CONFIG,
    content: formatContentWithTitle(content),
    allowHTML: true,
